# DESIGN.md — Slacc Design System Specification
...
- **`button-primary-pill`** — the dominant CTA system-wide.
  - Background `{colors.primary}` (`#4a154b`), text `{colors.on-primary}` (`#ffffff`), type `{typography.button-md}`, padding `14px 28px`, rounded `{rounded.pill}` `90px`.
  - Pressed state `button-primary-pill-pressed` shifts background to `{colors.primary-press}` (`#611f69`).
- **`button-secondary-pill`** — the soft lavender alternative.
  - Background `{colors.canvas-lavender}` (`#f9f0ff`), text `{colors.ink}` (`#1d1d1d`), padding `10px 30px`, same pill geometry. Used as the second action beside the primary aubergine pill.
