...
import { CameraLivenessVerification } from '@/components/verification/CameraLivenessVerification'
...
export default function LaporBaruPage() {
  ...
  const [verificationMethod, setVerificationMethod] = useState<'otp' | 'camera'>('otp')
  ...
  const [cameraLivenessScore, setCameraLivenessScore] = useState<number | null>(null)
  const [cameraSpoofRisk, setCameraSpoofRisk] = useState<number | null>(null)
  const [cameraQualityScore, setCameraQualityScore] = useState<number | null>(null)
    ...
    try {
      ...
      const res = await fetch('/api/reports', {
        ...
        body: JSON.stringify({
          ...
          email_verified: verificationMethod === 'otp' ? verified : false,
          verification_method: verificationMethod === 'camera' ? 'camera_liveness' : 'otp',
          verification_status: (verificationMethod === 'camera' && cameraVerified) || verified ? 'verified' : 'pending',
          verification_photo_url: verificationMethod === 'camera' ? cameraPhotoUrl : null,
          verification_timestamp: new Date().toISOString(),
          liveness_score: verificationMethod === 'camera' ? cameraLivenessScore : null,
            ...
            <div className="p-5 sm:p-8">
              ...
              {currentStep === 4 && (
                <div className="flex flex-col gap-5 animate-in fade-in duration-150">
                  ...
                  {/* Verification Method Toggle: OTP vs Camera Liveness */}
                  <div className="flex flex-col gap-2">
                    <span className="text-xs font-bold text-[#1d1d1d] uppercase tracking-wider font-mono">
                      2. Pilih Metode Verifikasi Pelapor
                    </span>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <button
                        type="button"
                        ...
                        className={`p-3.5 rounded-xl border flex items-center gap-3 text-left transition-all cursor-pointer ${
                          verificationMethod === 'camera'
                            ? 'bg-[#ecfdf5] border-[#007a5a] shadow-xs'
                  ...
                  {/* METHOD A: CAMERA LIVENESS */}
                  {verificationMethod === 'camera' && (
                    <div className="space-y-3">
                      <CameraLivenessVerification
                        onVerified={(data) => {
                          setCameraPhotoUrl(data.photoDataUrl)
                          setCameraLivenessScore(data.livenessScore)
                          setCameraSpoofRisk(data.spoofRisk)
                      ...
                      {cameraVerified && (
                        <button
                          type="button"
                          onClick={() => submitFinalReport(false)}
                          disabled={isSubmitting}
                          className="w-full min-h-[48px] px-8 py-3 rounded-xl bg-[#007a5a] hover:bg-[#006046] text-white font-bold text-sm flex items-center justify-center gap-2 transition-all cursor-pointer shadow-md disabled:opacity-50"
                        >
                          {isSubmitting ? (
                            <>
                              <RefreshCw className="w-4 h-4 animate-spin" />
                              <span>Mengirim Laporan Terverifikasi...</span>
                            </>
                          ) : (
                            <>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-bold text-[#4a154b] uppercase tracking-wider font-mono">
                            Verifikasi Email Pelapor (Supabase OTP)
                          </span>
                        </div>
                        {isEmailVerified ? (
