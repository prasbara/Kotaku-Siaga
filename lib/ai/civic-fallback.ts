...
Diperbarui: ${ctx.timestampWib}`
  ...
  const lines: string[] = [
    ...
    `${statusSummary}`,
    '',
    `🌧️ Curah Hujan: ${rainInfo}`,
    `💧 Observasi Genangan: ${ctx.reports.summary}`,
    `📹 CCTV PantauSemar: ${cctvInfo}`,
    `⚠️ Indeks Risiko Wilayah: ${ctx.risk.riskLevel} (${ctx.risk.riskScore.toFixed(1)}/100)`,
    '',
    `ℹ️ Catatan Sistem:`,
    `${ctx.risk.note}`,
    '',
